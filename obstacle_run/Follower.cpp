...
Follower::Follower()
{
	...
	m_FollowMaxFBStep = 30;
    m_FollowMinFBStep = -25.0;
...
void Follower::setReverse(bool reverseValue) {
	reverse = reverseValue;
}

void Follower::setWalk(int value){
	
	m_FBStep = value; 
}

void Follower::Process(Point2D ball_pos)
{
	if(DEBUG_PRINT == true)
		fprintf(stderr, "\r                                                                               \r");
	...
	else
	{
		...
		if(!reverse) {
			...
			if(m_RLTurn < m_GoalRLTurn){
				cout << "-2" << endl;
				cout << m_RLTurn << " " << m_GoalRLTurn << endl;
				m_RLTurn = -2;
			}
			else if(m_RLTurn > m_GoalRLTurn){
				cout << "-6" << endl;
