...
int GenericVision::getMoveStrategy()
{
	...
	int step = 0;
	cout << "rows " << this->threshold1Frame.rows << " cols " << this->threshold1Frame.cols << endl;
	...
	cout  << "All data " << topLeft << " " << bottomLeft << " " << topCenter << " " << bottomCenter << " " << topRight << " " << bottomRight  << endl;
	if(bottomCenter != 0 && bottomRight != 0)
	{
		step = 3 ; // left
	}
	else if(bottomCenter != 0 && bottomLeft != 0)
	{
		step = 4 ; // right
	}
	else if(topCenter != 0 && topRight != 0)
	{
		cout << " this is the culprit \n";
		step = 3 ; // left
	}
	else if(topCenter != 0 && topLeft != 0)
	{
		step = 4 ; // right
	}
	else if(topCenter !=0 && topRight == 0)
...
//=======================================================================================
// HERE and BELOW -> VISION PART(Kyle Ahn)
//=======================================================================================
//======================================================================================
// Author: Kyle Ahn
...
float GenericVision::getVisionAngle(string feature){
	...
	{
		Mat img = preprocessImgForLSD();
		std::vector<KeyLine> keyLines, mergedLines, singleLine;
		getLineSegments(img, keyLines);
		cleanUpLines(keyLines, mergedLines);
		detectSingleLine( mergedLines, singleLine);
		KeyLine defaultLine;
		defaultLine.startPointX = 0; defaultLine.startPointY = 299; defaultLine.endPointX = 299; defaultLine.endPointY = 299;
		angle = calcAngleBetweenTwoLines( defaultLine, singleLine[0]);
	}
